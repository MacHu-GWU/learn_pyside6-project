...
特殊的是, 在这个例子中, 我们用的是 ``QButtonGroup.buttonToggled`` 的 Signal. 在这个例子中
我们发现该方法失败了, 因为在一个 QButtonGroup 有几个 radio, 当任何 radio 改变时, 这个
Signal 就会被触发几次. 我们还是需要用 ``QRadioButton.toggled`` 的 Signal 来处理比较好.
"""
...
class MainWidget(QtWidgets.QWidget):
    ...
    def __init__(self, parent):
        ...
        self.button_group_1.buttonToggled.connect(self.button_group_1_toggled_handler)
        ...
        self.button_group_2.buttonToggled.connect(self.button_group_2_toggled_handler)
    ...
    def button_group_1_toggled_handler(self):
        print("trigger button_group_1_toggled_handler")
        button_group: QtWidgets.QButtonGroup = self.sender()
        for radio_wgt in button_group.buttons():
            if radio_wgt.isChecked():
                self.print_radio_button_status()

    @QtCore.Slot()
    def button_group_2_toggled_handler(self):
        print("trigger button_group_2_toggled_handler")
        button_group: QtWidgets.QButtonGroup = self.sender()
        for radio_wgt in button_group.buttons():
            if radio_wgt.isChecked():
                self.print_radio_button_status()
